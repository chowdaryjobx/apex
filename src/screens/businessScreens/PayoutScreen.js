...
function PayoutScreen({navigation}) {
  const {authUser, user, userData, logOut, api, url, fonts} = React.useContext(
    DataContext,
  );
  if (!user) {
  ...
  useEffect(() => {
    axios
      .post(api + url.WithdrawPayouts, {PageIndex: 1, TokenID: user.TokenId})
      .then(res => {
  ...
  if (!isLoading) {
    ...
    return (
      <View style={{flex: 1, justifyContent: 'center', alignItems: 'center'}}>
        <Text style={{fontFamily: fonts.BOLD}}>Loading ...</Text>
        <Text style={{color: 'red', fontFamily: fonts.BOLD}}>{errMessage}</Text>
      </View>
    );
  }
}
